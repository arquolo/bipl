...
def _apply(fn: Callable[[np.ndarray], np.ndarray], obj: Tile) -> Tile:
    r = fn(obj.data)
    assert r.shape[:2] == obj.data.shape[:2], 'Callable changed data shape'
...
def _apply_batched(
    ...
    assert len(rs) == len(ts), 'Input & output has different sizes'

    r_shapes = [r.shape[:2] for r in rs]
    t_shapes = [t.data.shape[:2] for t in ts]
    assert r_shapes == t_shapes, 'Tile height/width should not change'
...
class Mosaic:
    ...
    def __post_init__(self):
        ...
        if self.overlap > self.step:
            raise ValueError(
                'overlap should be lower than step, got: '
    ...
    def iterate(self, image: NumpyLike, max_workers: int = 1) -> '_ArrayTiles':
        ...
        # Index
        ih, iw = ishape = tuple(
            (s + self.tile_size - 1) // self.step for s in shape
        )
        cells = np.ones(ishape, dtype=np.bool_)

        # Align slide & cells centers
        view = tuple(round(i * self.step + self.overlap) for i in ishape)
        origin = tuple((s - v) // 2 for s, v in zip(shape, view))
...
class Patches:
    ...
    def map(
        self, fn: Callable[[np.ndarray], np.ndarray], /, max_workers: int = 0
    ) -> Self:
        """
    ...
    def pool(self, stride: int = 1) -> 'Patches':
        ...
        if stride == 1:
            return self
        shape = tuple((s + stride - 1) // stride for s in self.shape)
...
class _ArrayTiles(Tiles):
    ...
    def select(
        self, mask: np.ndarray, scale: float | None = None
    ) -> '_ArrayTiles':
        """
        ...
        view = tuple(round(i * self.m.step * scale) for i in ishape)
        loc = tuple(
            ((s0 - s1) // 2, (s0 - s1) // 2 + s1)
    ...
    def _drop_overlaps(
        self, grid: npt.NDArray[np.int64]
    ...
    def __iter__(self) -> Iterator[Tile]:
        ...
        if isinstance(self.data, HasPartsAbc):
            ...
            parts = (
                Tile(idx=i, vec=tuple(s[0] for s in p.loc), data=p.data)
